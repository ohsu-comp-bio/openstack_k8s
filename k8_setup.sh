...
if [ ! -f kubeadm.out ]; then
  ...
  KUBEADM_RESULTS=$(ssh -i ~/.ssh/$KEYPAIR_NAME ubuntu@${SERVER_MAP[$SERVER_NAME]} 'sudo kubeadm init --apiserver-advertise-address '${SERVER_MAP[$SERVER_NAME]})
  if [ $? -eq 0 ]; then
      echo "OK"
  else
  ...
  cat << EOF | $(ssh -i ~/.ssh/$KEYPAIR_NAME ubuntu@${SERVER_MAP[$SERVER_NAME]})
...
do
  ...
  if [ ! -f kubeadm-$SERVER_NAME.out ]; then
    ...
    OK=OK
    KUBEADM_RESULTS=$(ssh -i ~/.ssh/$KEYPAIR_NAME ubuntu@${SERVER_MAP[$SERVER_NAME]} 'sudo '$JOIN)
    if [ $? -eq 0 ]; then
        echo "OK"
    else
        echo kubeadm $SERVER_NAME FAIL
